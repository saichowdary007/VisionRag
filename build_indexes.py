...
from rag_service.embeddings.image_embedder import SiglipImageEmbedder
...
def insert_images(conn: sqlite3.Connection, images: List[tuple[str, str, int, str, str]]) -> List[int]:
    """Insert images as (image_id, doc_id, page, path, bbox_json) and return rowids."""
    if not images:
        return []
    cur = conn.cursor()
    for rec in images:
...
def main():
    ...
    for pdf in pdfs:
        ...
        # Extract and insert images with bboxes (document-level to avoid duplication)
    ...
    if cfg.enable_vision and all_image_paths and img_embedder is not None:
        ...
        if img_embs.shape[0] == len(all_image_rowids):
            ...
            build_faiss_index(img_embs, np.array(all_image_rowids, dtype=np.int64), Path(cfg.faiss_image_index_path))
            print(f"Wrote image index to {cfg.faiss_image_index_path}")
        else:
