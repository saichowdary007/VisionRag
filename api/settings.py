import os
from functools import lru_cache
class Settings:
    # Directories
    DATA_DIR: str = os.getenv("DATA_DIR", "/data")
    # Service bases
    RETRIEVER_BASE_URL: str = os.getenv("RETRIEVER_BASE_URL", "http://retriever:8081")
    LMSTUDIO_BASE_URL: str = os.getenv("LMSTUDIO_BASE_URL", "http://vlm:1234")
    LMSTUDIO_API_KEY: str = os.getenv("LMSTUDIO_API_KEY", "lm-studio")
    LMSTUDIO_MODEL: str = os.getenv("LMSTUDIO_MODEL", "google/gemma-3-12b-it")
    # API base URLs
    # Used to construct image URLs accessible to LM Studio (inside the Docker network)
    API_INTERNAL_BASE: str = os.getenv("API_INTERNAL_BASE", "http://api:8080")
    # Used by the browser
    API_PUBLIC_BASE: str = os.getenv("API_PUBLIC_BASE", "http://localhost:8080")
    # CORS
    WEB_ORIGIN: str = os.getenv("WEB_ORIGIN", "http://localhost:5173")
    # Rendering sizes
    PAGE_MAX_DIM: int = int(os.getenv("PAGE_MAX_DIM", "1024"))
    VLM_SQUARE: int = int(os.getenv("VLM_SQUARE", "896"))
@lru_cache()
def get_settings() -> Settings:
    return Settings()
